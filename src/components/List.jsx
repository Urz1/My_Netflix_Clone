...
const List = () => {
  ...
  const handleClick = (direction) => {
    ...
    if (direction === "left") {
      ...
      listRef.current.style.transform = `translateX(${290 * (slideNumber === 0 ? -(totalItems - visibleItems) : 1) + distance}px)`;
    } else if (direction === "right") {
      const newSlideNumber = slideNumber === totalItems - visibleItems ? 0 : slideNumber + 1;
      setSlideNumber(newSlideNumber);
      listRef.current.style.transform = `translateX(${290 * (slideNumber === totalItems - visibleItems ? (totalItems - visibleItems) : -1) + distance}px)`;
    }
  };
  return (
    <div className="">
      <div className="flex flex-nowrap items-center m-3 relative">
        ...
        />
        <div className="flex m-2 w-max p-4 transition-transform duration-300 ease-in-out " ref={listRef}>
          <ListItem />
          <ListItem />
          <ListItem />
          <ListItem />
